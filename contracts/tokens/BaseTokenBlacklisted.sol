...
contract BaseTokenBlacklisted is IERC20, IBaseToken {
    ...
    modifier outBlacklisted() {
        require(!isBlacklisted[msg.sender], "you already blacklisted");
        _;
    }

    modifier outBlacklistedAccount(address _account) {
    ...
    function addBlacklist(address _account) external onlyGov {
        require(!isBlacklisted[_account], "account already blacklisted");
        isBlacklisted[_account] = true;
    }

    function removeBlacklist(address _account) external onlyGov {
        require(isBlacklisted[_account], "account is outside the blacklist");
    ...
    function transfer(address _recipient, uint256 _amount) external override outBlacklisted outBlacklistedAccount(_recipient) returns (bool) {
    ...
    function approve(address _spender, uint256 _amount) external override outBlacklisted outBlacklistedAccount(_spender) returns (bool) {
        _approve(msg.sender, _spender, _amount);
        return true;
    }

    function transferFrom(address _sender, address _recipient, uint256 _amount) external override outBlacklisted outBlacklistedAccount(_sender) outBlacklistedAccount(_recipient) returns (bool) {
