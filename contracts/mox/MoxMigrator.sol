...
contract MoxMigrator is ReentrancyGuard, IGmxMigrator {
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "MoxMigrator: forbidden");
        _;
    }

    modifier onlySigner() {
        require(isSigner[msg.sender], "MoxMigrator: forbidden");
        _;
    }
    ...
    function initialize(
        ...
        require(!isInitialized, "MoxMigrator: already initialized");
        require(_whitelistedTokens.length == _iouTokens.length, "MoxMigrator: invalid _iouTokens.length");
        require(_whitelistedTokens.length == _prices.length, "MoxMigrator: invalid _prices.length");
        require(_whitelistedTokens.length == _caps.length, "MoxMigrator: invalid _caps.length");
        require(_lpTokens.length == _lpTokenAs.length, "MoxMigrator: invalid _lpTokenAs.length");
        require(_lpTokens.length == _lpTokenBs.length, "MoxMigrator: invalid _lpTokenBs.length");
    ...
    function migrate(
        ...
        require(isMigrationActive, "MoxMigrator: migration is no longer active");
        require(whitelistedTokens[_token], "MoxMigrator: token not whitelisted");
        require(_tokenAmount > 0, "MoxMigrator: invalid tokenAmount");

        if (hasMaxMigrationLimit) {
            migratedAmounts[msg.sender][_token] = migratedAmounts[msg.sender][_token].add(_tokenAmount);
            require(migratedAmounts[msg.sender][_token] <= maxMigrationAmounts[msg.sender][_token], "MoxMigrator: maxMigrationAmount exceeded");
        }

        uint256 tokenPrice = getTokenPrice(_token);
        uint256 mintAmount = _tokenAmount.mul(tokenPrice).div(gmxPrice);
        require(mintAmount > 0, "MoxMigrator: invalid mintAmount");

        tokenAmounts[_token] = tokenAmounts[_token].add(_tokenAmount);
        require(tokenAmounts[_token] < caps[_token], "MoxMigrator: token cap exceeded");
        ...
        if (lpTokens[_token]) {
            ...
            require(tokenA != address(0), "MoxMigrator: invalid tokenA");
            require(tokenB != address(0), "MoxMigrator: invalid tokenB");
    ...
    function signApprove(address _token, address _spender, uint256 _amount, uint256 _nonce) external nonReentrant onlySigner {
        ...
        require(!signedActions[msg.sender][action], "MoxMigrator: already signed");
    ...
    function getTokenPrice(address _token) public view returns (uint256) {
        ...
        require(price != 0, "MoxMigrator: invalid token price");
        return price;
    }

    function getIouToken(address _token) public view returns (address) {
        address iouToken = iouTokens[_token];
        require(iouToken != address(0), "MoxMigrator: invalid iou token");
        return iouToken;
    }
    ...
    function _validateAction(bytes32 _action) private view {
        require(pendingActions[_action], "MoxMigrator: action not signalled");
    }
    ...
    function _validateAuthorization(bytes32 _action) private view {
        ...
        if (count == 0) {
            revert("MoxMigrator: action not authorized");
        }
        require(count >= minAuthorizations, "MoxMigrator: insufficient authorization");
    }

    function _clearAction(bytes32 _action, uint256 _nonce) private {
        require(pendingActions[_action], "MoxMigrator: invalid _action");
