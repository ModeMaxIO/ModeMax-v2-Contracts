...
contract PositionRouterSide is BasePositionManager, IPositionRouter {
    ...
    function createIncreasePositionTpSl(
        address _account,
    ...
    ) external payable nonReentrant onlyPositionKeeper returns (bytes32) {
    ...
    function createIncreasePositionETHTpSl(
        address _account,
        address[] memory _path,
        address _indexToken,
        uint256 _minOut,
    ...
    function _setTraderReferralCode(address _account, bytes32 _referralCode) internal {
        ...
        if (_referralStorage.traderReferralCodes(_account) != bytes32(0)) { return; }

        _referralStorage.setTraderReferralCode(_account, _referralCode);
    }
