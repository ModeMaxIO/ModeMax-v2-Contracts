...
import "./interfaces/IAdapterPyth.sol";
...
contract ChainlinkAggregator4Pyth is Owned, AggregatorV2V3Interface {
    ...
    constructor(uint8 _decimals, string memory _description, address _adapter, bytes32 _id) {
        decimals = _decimals;
    ...
    function latestAnswer() external view returns (int256) {
        if (adapterAddress != address(0)) {
            (int256 price,) = IAdapterPyth(adapterAddress).latestPrice(priceFeedId, decimals);
            return price;
        }
    ...
    function latestTimestamp() external view returns (uint256) {
        if (adapterAddress != address(0)) {
            (, uint256 timestamp) = IAdapterPyth(adapterAddress).latestPrice(priceFeedId, decimals);
            return timestamp;
        }
    ...
    function latestAnswerTimestampOracle() external view returns (int256, uint256) {
        if (adapterAddress != address(0)) {
            (int256 price, uint256 timestamp) = IAdapterPyth(adapterAddress).latestPrice(priceFeedId, decimals);
    ...
    )
    {
        if (adapterAddress != address(0)) {
            (int256 price, uint256 timestamp) = IAdapterPyth(adapterAddress).latestPrice(priceFeedId, decimals);
            return (latestAggregatorRoundId, price, timestamp, timestamp, latestAggregatorRoundId);
        }
